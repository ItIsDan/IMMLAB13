...
namespace modelingLab2
{
    public partial class Form1 : Form
    {
        ...
        double sigma = 0.15; // Параметр волатильности
        ...
        {
            rateEuro += sign() * Math.Exp((mu - 0.5 * sigma * sigma) * dt + sigma * sigma * Math.Sqrt(dt) * NextGaussian()); 
            rateDollar += sign() * Math.Exp((mu - 0.5 * sigma * sigma) * dt + sigma * sigma * Math.Sqrt(dt) * NextGaussian());
